...
O sistema foi desenhado visando alta performance, isolamento de domínio e baixo custo. Ele é dividido em três camadas:
...
2. **Static Data Layer (GitHub Pages):** Os arquivos JSON fragmentados são hospedados no GitHub Pages. Isso atua como um banco de dados estático e altamente escalável, distribuído globalmente via CDN.
...
**Endpoint:**
```http
GET https://<SEU-DOMINIO-WORKER>.workers.dev/{mac-address}
...
```bash
curl https://<SEU-DOMINIO-WORKER>.workers.dev/a8:23:fe:08:37:ba
...
```bash
curl https://<SEU-DOMINIO-WORKER>.workers.dev/00:50:C2:4B:41:00
...
```bash
curl https://<SEU-DOMINIO-WORKER>.workers.dev/invalid-mac
